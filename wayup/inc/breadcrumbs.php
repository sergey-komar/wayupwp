...
function wayup_get_breadcrumbs() {
    ...
    $text['home']     = __('Главная','wayup');
    $text['category'] = __('Archive','wayup').' "%s"';
    $text['search']   = __('Результаты поиска','wayup').' "%s"';
    ...
    $show_home_link = 1;
    $show_title     = 1;
    // МЕНЯЕМ СЕПАРАТОР   МЕНЯЕМ СЕПАРАТОР   МЕНЯЕМ СЕПАРАТОР 
    $delimiter      = '&nbsp;<span class="sep">/</span>&nbsp;';
     // МЕНЯЕМ СЕПАРАТОР   МЕНЯЕМ СЕПАРАТОР   МЕНЯЕМ СЕПАРАТОР
     
    //  ДОБАВЛЯЕМ СВОЙ КЛАСС ДОБАВЛЯЕМ СВОЙ КЛАСС
    ...
    if (is_home() || is_front_page()) {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo __('&nbsp&nbspСтраница') . ' ' . get_query_var('paged');
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
